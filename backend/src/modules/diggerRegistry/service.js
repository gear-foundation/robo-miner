...
export class DiggerRegistryService {
  ...
  async register({ programId, owner = null, seasonId = null, worldId = null, status = 'active', source = 'manual' }) {
    ...
    return this.store.update((db) => {
      ...
      const next = {
        ...
        seasonId: seasonId || existing?.seasonId || this.config.diggerRentalSeason,
        worldId: worldId ?? existing?.worldId ?? null,
  ...
  async list({ seasonId = null, worldId = null, owner = null, status = null } = {}) {
    ...
    return db.diggers
      ...
      .filter((digger) => !worldId || digger.worldId === worldId)
      .filter((digger) => !normalizedOwner || normalizeAddress(digger.owner) === normalizedOwner)
