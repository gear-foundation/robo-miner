...
// redundant top-ups while one is still settling.
...
export function createBalanceKeeper({ chain, log = console.log, clock = Date.now, options = {} }) {
  ...
  const minVara = options.minVara ?? 150;
  const topUpVara = options.topUpVara ?? 400;
  ...
  async function check(programId, s) {
    ...
    if (eb < minWei && now - s.lastTopUpAt >= cooldownMs) {
      ...
      await chain.topUpExecutableBalance(programId, topUpWei);
      s.lastTopUpAt = clock();
