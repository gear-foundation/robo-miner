...
// Live DiggerWorld contract tile ids observed from MapSnapshot() on Hoodi.
...
function encodeTile(renderTile, y, renderSurface, contractSurface) {
  if (y < contractSurface) return CONTRACT_TILE.SKY;
  if (y < renderSurface) return CONTRACT_TILE.DIRT;

  switch (renderTile) {
    case BLOCK.SKY: return CONTRACT_TILE.CAVE;
...
function validateContractMap(world, map, opts = {}) {
  ...
  for (let y = 0; y < contractSurface; y++) {
    for (let x = 0; x < world.W; x++) {
      if (map[y * world.W + x] !== CONTRACT_TILE.SKY) {
        warnings.push(`non-sky tile above surface at ${x},${y}`);
        x = world.W;
        y = contractSurface;
      }
    }
